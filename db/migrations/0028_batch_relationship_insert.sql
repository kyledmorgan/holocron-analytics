...
--   - Accepts a JSON array of relationship records
--   - Validates relationship data
--   - Resolves entity names to entity IDs (best-effort, case-insensitive)
...
-- Accepts JSON array and inserts relationships into BridgeEntityRelation
-- ============================================================================
IF OBJECT_ID('[dbo].[usp_batch_insert_entity_relations]', 'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [dbo].[usp_batch_insert_entity_relations];
END
GO
...
PRINT 'Stored procedure [dbo].[usp_batch_insert_entity_relations] created successfully.'
GO


-- ============================================================================
-- Stored Procedure: dbo.usp_get_entity_relations
-- Query relationships for an entity
-- ============================================================================
IF OBJECT_ID('[dbo].[usp_get_entity_relations]', 'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [dbo].[usp_get_entity_relations];
END
GO

CREATE PROCEDURE [dbo].[usp_get_entity_relations]
    @entity_id UNIQUEIDENTIFIER = NULL,       -- Entity ID to find relationships for
    @entity_name NVARCHAR(500) = NULL,        -- Or entity name (case-insensitive)
    @relation_type NVARCHAR(100) = NULL,      -- Optional filter by relation type
    @direction NVARCHAR(10) = 'both',         -- 'from', 'to', or 'both'
    @limit INT = 100
AS
BEGIN
    SET NOCOUNT ON;
    
    -- Resolve entity_name to entity_id if provided
    IF @entity_id IS NULL AND @entity_name IS NOT NULL
    BEGIN
        SELECT TOP 1 @entity_id = EntityId
        FROM [dbo].[DimEntity]
        WHERE LOWER(DisplayName) = LOWER(@entity_name)
          AND IsLatest = 1
          AND IsActive = 1;
    END
    ...
    BEGIN
        SELECT 'ERROR' AS status, 'Entity not found' AS error_message;
        RETURN;
    END
    ...
    SELECT TOP (@limit)
        ber.RelationId,
        ber.FromEntityId,
        from_e.DisplayName AS FromEntityName,
        from_e.EntityType AS FromEntityType,
        ber.ToEntityId,
        to_e.DisplayName AS ToEntityName,
        to_e.EntityType AS ToEntityType,
        ber.RelationType,
        ber.Confidence,
        ber.StartDate,
        ber.EndDate,
    ...
    LEFT JOIN [dbo].[DimEntity] from_e ON ber.FromEntityId = from_e.EntityId AND from_e.IsLatest = 1
    LEFT JOIN [dbo].[DimEntity] to_e ON ber.ToEntityId = to_e.EntityId AND to_e.IsLatest = 1
    WHERE ber.IsActive = 1
      AND (
          (@direction = 'both' AND (ber.FromEntityId = @entity_id OR ber.ToEntityId = @entity_id))
          OR (@direction = 'from' AND ber.FromEntityId = @entity_id)
          OR (@direction = 'to' AND ber.ToEntityId = @entity_id)
      )
      AND (@relation_type IS NULL OR ber.RelationType = @relation_type)
    ORDER BY ber.CreatedUtc DESC;
END
GO

PRINT 'Stored procedure [dbo].[usp_get_entity_relations] created successfully.'
GO


-- ============================================================================
-- Stored Procedure: dbo.usp_count_relations_by_type
-- Count relationships by type
-- ============================================================================
IF OBJECT_ID('[dbo].[usp_count_relations_by_type]', 'P') IS NOT NULL
BEGIN
    DROP PROCEDURE [dbo].[usp_count_relations_by_type];
END
GO
...
PRINT 'Stored procedure [dbo].[usp_count_relations_by_type] created successfully.'
GO

PRINT 'Migration 0028 completed: Batch relationship insert stored procedures created.'
GO
