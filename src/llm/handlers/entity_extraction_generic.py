...
Supports: PersonCharacter, Droid, Species, LocationPlace, VehicleCraft,
ObjectItem, Organization, Concept, EventConflict, TimePeriod, WorkMedia, etc.
"""
...
from ..interrogations.definitions.entity_extraction_generic import (
...
class EntityExtractionGenericHandler:
    """
    Handler for generic entity extraction jobs.
    
    This handler:
    - Extracts entities of ANY type from source text using LLM
    - Validates output against generic entity extraction contract
    ...
    def interrogation(self):
        ...
        if self._interrogation is None:
            self._interrogation = get_interrogation("entity_extraction_generic_v1")
        return self._interrogation
    
    def handle(self, job: Job, ctx: RunContext) -> HandlerResult:
        """
        Handle a generic entity extraction job.
        ...
        metrics: Dict[str, Any] = {
            "handler": "entity_extraction_generic",
            "execution_mode": ctx.execution_mode.value,
        }
        
        log_ctx = ctx.get_log_context()
        logger.info(f"Starting generic entity extraction for job {job.job_id}", extra=log_ctx)
        ...
        try:
            ...
            # Optional: type focus from job input
            type_focus = job_input.get("type_focus", None)
            
            metrics["source_id"] = source_id
            metrics["content_length"] = len(content)
            if type_focus:
                metrics["type_focus"] = type_focus
            ...
            validation_errors = validate_entity_extraction_generic_output(parsed_output)
            ...
            metrics["entity_types_found"] = list(entity_types)
        ...
        for entity in entities:
            try:
                write_record = self.entity_store.upsert_entity(
                    ...
                    work_references=entity.get("work_references"),
                    run_id=ctx.run_id,
                    job_id=ctx.job_id,
                )
